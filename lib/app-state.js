...
const APP_SETTINGS_DEFAULTS = {
  ...
  chatViewMode: 'tabs',
  chatGridSize: 3, // max vertical columns side-by-side
  channelOrderByAccount: {}
};
...
function mergeAppSettings(rawSettings) {
  ...
  return {
    ...
    channelOrderByAccount: Object.keys(asObject(raw.channelOrderByAccount)).reduce(function (acc, accountId) {
      const list = raw.channelOrderByAccount[accountId];
      acc[accountId] = Array.isArray(list)
        ? list.map(function (item) { return String(item || '').toLowerCase().trim(); }).filter(Boolean)
        : [];
      return acc;
    }, {})
  };
